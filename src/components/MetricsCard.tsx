...
export function MetricsCard({ metrics, symbol }: MetricsCardProps) {
  ...
  return (
    <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
      <h3 className="text-sm font-semibold text-slate-300 mb-3">Metrics — {symbol}</h3>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div>
          <div className="text-xs text-slate-500">YoY Return ({latestYear || '—'})</div>
      ...
      {years.length > 1 && (
        <div className="mt-3 pt-3 border-t border-slate-700">
          <div className="text-xs text-slate-500 mb-1">Yearly Returns</div>
          <div className="flex flex-wrap gap-2">
            {years.slice(-5).map((y) => (
              <span key={y} className="text-xs px-2 py-0.5 rounded bg-slate-700 text-slate-300">
